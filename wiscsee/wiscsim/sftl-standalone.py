...
if __name__ == "__main__":
    trans_pages = {}
    trace = "/home/js39/datasets/MSR-Cambridge/usr_0.csv"
    ...
    writes = parse_events(trace, lineno=10000000)
    # writes = [60 + () for i in range(256)]
    # writes = [(lpn, i) for i, lpn in enumerate(writes)]
    #writes = [(0, 2), (2, 2), (4, 2), (6, 2), (2, 6)]
    buffer = SimpleWriteBuffer(256)
    exist_lpns = defaultdict(int)
    for i, write in enumerate(writes):
        for lpn in range(write[0], write[0]+write[1]):
            buffer.write(lpn)
            exist_lpns[lpn] = ""

        if i % 100000 == 0:
            print("%d write requests" % i)
            print("SFTL Memory consumption in Byte is:", sum([x.memory_size for x in trans_pages.values()]))
            print("Fully-cached DFTL Memory Consumption is :", len(exist_lpns)*4)
        
        if not buffer.should_flush():
            continue

        all_writes = buffer.flush()

        all_writes = group_consecutives(all_writes)
        
        for write in all_writes:
            # print(split_write_to_page(write[0], write[1]))
            for singlewrite in split_write_to_page(write[0], write[1]):
                if (singlewrite[0] // trans_page_entry) not in trans_pages:
                    trans_pages[singlewrite[0] //
                                trans_page_entry] = trans_page(singlewrite[0] // trans_page_entry)
                trans_pages[singlewrite[0] //
                            trans_page_entry].write(singlewrite[0] % trans_page_entry, singlewrite[1])
