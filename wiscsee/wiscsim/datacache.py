...
if __name__ == "__main__":
    from wiscsim.workload_parser import parse_events, mix_events, create_config, split_ext
    from workflow import *

    trace = "/home/js39/datasets/MSR-Cambridge/rsrch_0.csv"
    log_msg("Start parsing", trace)
    events = parse_events(trace, page_size=4096, recorder=False)
    log_msg("End parsing", trace)
    conf = create_config(ftl_type="learnedftl")
    operations = []
    for event in events:
        if event.get_operation() == OP_WRITE:
            mode = "write"
        elif event.get_operation() == OP_READ:
            mode = "read"
        else:
            continue
        exts = event.get_lpn_extent(conf)
        for ext in split_ext(exts):
            operations += [(mode, ext)]
    
    buffer = RWCache(500*MB, 4096, 8*MB, 8.0/8.0)
    for i, (mode, ext) in enumerate(operations):
        lpn = ext.lpn_start
        if mode == "write":
            writeback, ppn = buffer.write(lpn)
            # if writeback:
            #     print("writeback", ppn)
            #print(len(buffer.assigned))
        else:
            hit, writeback, ppn = buffer.read(lpn)
        
        if buffer.should_assign_page():
            lpns = buffer.flush_unassigned()
            #print("assign lpns:", lpns)
            buffer.update_assigned({lpn : lpn for lpn in lpns})
        
        # if i != 0 and i % 100000 == 0:
        #     print(i)
        #     buffer.record_counter()

    buffer.record_counter()


    # cache = DataCache(8192, 4096, method="LRU")
    # sequence = [10,20,20,30,30,40,40,50,10,60]
    # for lpn in sequence:
    #     cache.read(lpn)
    #     print(cache.cache)

    # buffer = WriteBuffer(1*256, 1*256)
    # sequence = [_**2 for _ in range(512)]
    # for lpn in sequence:
    #     writeback, ppn = buffer.write(lpn)
    #     print("writeback", ppn)
    #     if buffer.should_assign_page():
